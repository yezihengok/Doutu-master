...
public class StatusBarUtil {
    ...
    public static final int DEFAULT_STATUS_BAR_ALPHA = 100;
    ...
    public static void setDecorView(Activity activity) {
        // 安卓系统7.0时，状态栏为透明的，有一层朦层问题
        //从源码可以看到androidx.x之后会强制绘制statusbar的背景色, 而且在其他条件都满足情况下还会加上半透明效果,
        // 所以如果第一种方法去除不掉半透明的情况下可以通过反射拿到mSemiTransparentStatusBarColor这个属性, 然后实现全透明效果, 代码如下:
    ...
    public static int  getStatusBarHeight() {
        ...
        try {
            int resourceId = DouApplication.getInstance().getResources().getIdentifier("status_bar_height", "dimen", "android");
            if (resourceId > 0) {
                statusBarHeight = DouApplication.getInstance().getResources().getDimensionPixelSize(resourceId);
            }
        } catch (Resources.NotFoundException e) {
            e.printStackTrace();
        }

        //处理部分手机（例如华为）获取的状态栏高度，比实际显示的高度要大..这时取固定高度
        int barHeight= (int) getStatusBarHeights();
        if((statusBarHeight-barHeight)>5){
            statusBarHeight=barHeight;
        }
        Log.d("tag-","状态栏高度:" + statusBarHeight+" 固定状态栏高度---" + getStatusBarHeights()+" 实际取值状态栏高度=" + statusBarHeight);
        return statusBarHeight;
    }

    private static double getStatusBarHeights(){
        double statusBarHeight = Math.ceil(25 * DouApplication.getInstance().getResources().getDisplayMetrics().density);
        return statusBarHeight;
    }
